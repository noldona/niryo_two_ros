#include "niryo_one_hardware/ros_interface.h"

RosInterface::RosInterface(CommunicationBase *niryo_one_comm,
		RpiDiagnostics *rpi_diagnostics, bool *flag_reset_controllers,
		bool learning_mode_on, int hardware_version): Node("ros_interface") {
	comm = niryo_one_comm;
	...
	rpi_image_version = this->get_parameter("image_version").as_string();
	ros_niryo_one_version = this->get_parameter("ros_version").as_string();

	// trim string
	rpi_image_version.erase(rpi_image_version.find_last_not_of(" \n\r\t") + 1);
	ros_niryo_one_version.erase(
			ros_niryo_one_version.find_last_not_of(" \n\r\t") + 1);
...
bool RosInterface::callbackTestMotors(niryo_one_msgs::SetInt::Request &req,
...
bool RosInterface::callbackCalibrateMotors(niryo_one_msgs::SetInt::Request &req,
		niryo_one_msgs::SetInt::Response &res) {
...
bool RosInterface::callbackRequestNewCalibration(
 ...
 */
bool RosInterface::callbackActivateLearningMode(
		niryo_one_msgs::SetInt::Request &req,
		niryo_one_msgs::SetInt::Response &res) {
	...
	res.status = 200;
	res.message = (learning_mode_on) ? "Activating learning mode" :
									   "Deactivating learning mode";
	return true;
}

bool RosInterface::callbackActivateLeds(niryo_one_msgs::SetLeds::Request &req,
...
bool RosInterface::callbackPingAndSetDxlTool(
...
bool RosInterface::callbackPingAndSetConveyor(
...
bool RosInterface::callbackControlConveyor(
...
bool RosInterface::callbackUpdateIdConveyor(
...
bool RosInterface::callbackOpenGripper(
		niryo_one_msgs::OpenGripper::Request &req,
		niryo_one_msgs::OpenGripper::Response &res) {
	res.state = comm->openGripper(
			req.id, req.open_position, req.open_speed, req.open_hold_torque);
	return true;
}

bool RosInterface::callbackCloseGripper(
		niryo_one_msgs::CloseGripper::Request &req,
		niryo_one_msgs::CloseGripper::Response &res) {
	res.state = comm->closeGripper(req.id, req.close_position, req.close_speed,
			req.close_hold_torque, req.close_max_torque);
	return true;
}

bool RosInterface::callbackPullAirVacuumPump(
...
bool RosInterface::callbackPushAirVacuumPump(
...
bool RosInterface::callbackChangeHardwareVersion(
	...
	if (result == CHANGE_HW_VERSION_OK) {
		...
		res.status = 400;
		res.message =
				"Failed to change hardware version, please check the ROS logs";
...
bool RosInterface::callbackSendCustomDxlValue(
	...
	comm->addCustomDxlCommand(req.motor_type, req.id, req.value,
...
bool RosInterface::callbackRebootMotors(niryo_one_msgs::SetInt::Request &req,
		niryo_one_msgs::SetInt::Response &res) {
	comm->rebootMotors();
	res.status = 200;
	res.message = "OK";
...
void RosInterface::startServiceServers() {
	calibrate_motors_server = nh_.advertiseService("niryo_one/calibrate_motors",
			&RosInterface::callbackCalibrateMotors, this);
	request_new_calibration_server =
			nh_.advertiseService("niryo_one/request_new_calibration",
					&RosInterface::callbackRequestNewCalibration, this);

	test_motors_server = nh_.advertiseService(
			"niryo_one/test_motors", &RosInterface::callbackTestMotors, this);

	activate_learning_mode_server =
			nh_.advertiseService("niryo_one/activate_learning_mode",
					&RosInterface::callbackActivateLearningMode, this);
	activate_leds_server = nh_.advertiseService("niryo_one/set_dxl_leds",
			&RosInterface::callbackActivateLeds, this);

	ping_and_set_dxl_tool_server =
			nh_.advertiseService("niryo_one/tools/ping_and_set_dxl_tool",
					&RosInterface::callbackPingAndSetDxlTool, this);

	// steppers service test
	ping_and_set_stepper_server =
			nh_.advertiseService("niryo_one/kits/ping_and_set_conveyor",
					&RosInterface::callbackPingAndSetConveyor, this);
	control_conveyor_server =
			nh_.advertiseService("niryo_one/kits/control_conveyor",
					&RosInterface::callbackControlConveyor, this);
	update_conveyor_id_server =
			nh_.advertiseService("niryo_one/kits/update_conveyor_id",
					&RosInterface::callbackUpdateIdConveyor, this);

	open_gripper_server = nh_.advertiseService("niryo_one/tools/open_gripper",
			&RosInterface::callbackOpenGripper, this);
	close_gripper_server = nh_.advertiseService("niryo_one/tools/close_gripper",
			&RosInterface::callbackCloseGripper, this);
	pull_air_vacuum_pump_server =
			nh_.advertiseService("niryo_one/tools/pull_air_vacuum_pump",
					&RosInterface::callbackPullAirVacuumPump, this);
	push_air_vacuum_pump_server =
			nh_.advertiseService("niryo_one/tools/push_air_vacuum_pump",
					&RosInterface::callbackPushAirVacuumPump, this);

	change_hardware_version_server =
			nh_.advertiseService("niryo_one/change_hardware_version",
					&RosInterface::callbackChangeHardwareVersion, this);
	send_custom_dxl_value_server =
			nh_.advertiseService("niryo_one/send_custom_dxl_value",
					&RosInterface::callbackSendCustomDxlValue, this);
	reboot_motors_server = nh_.advertiseService("niryo_one/reboot_motors",
			&RosInterface::callbackRebootMotors, this);
}

void RosInterface::publishHardwareStatus() {
	double publish_hw_status_frequency;
	ros::param::get(
			"~publish_hw_status_frequency", publish_hw_status_frequency);
	ros::Rate publish_hardware_status_rate =
			ros::Rate(publish_hw_status_frequency);
	...
	while (ros::ok()) {
		...
		comm->getHardwareStatus(&connection_up, error_message,
				&calibration_needed, &calibration_in_progress, motor_names,
				motor_types, temperatures, voltages, hw_errors);
...
void RosInterface::publishSoftwareVersion() {
	...
	ros::param::get("~publish_software_version_frequency",
			publish_software_version_frequency);
	ros::Rate publish_software_version_rate =
			ros::Rate(publish_software_version_frequency);
...
void RosInterface::publishLearningMode() {
	...
	ros::param::get("~publish_learning_mode_frequency",
			publish_learning_mode_frequency);
	ros::Rate publish_learning_mode_rate =
			ros::Rate(publish_learning_mode_frequency);
...
void RosInterface::publishConveyor1Feedback() {
	...
	ros::Rate publish_conveyor_feedback_rate =
			ros::Rate(publish_conveyor_feedback_frequency);
...
void RosInterface::startPublishers() {
	hardware_status_publisher = nh_.advertise<niryo_one_msgs::HardwareStatus>(
			"niryo_one/hardware_status", 10);
	publish_hardware_status_thread.reset(new std::thread(
			boost::bind(&RosInterface::publishHardwareStatus, this)));

	software_version_publisher = nh_.advertise<niryo_one_msgs::SoftwareVersion>(
			"niryo_one/software_version", 10);
	publish_software_version_thread.reset(new std::thread(
			boost::bind(&RosInterface::publishSoftwareVersion, this)));

	learning_mode_publisher =
			nh_.advertise<std_msgs::Bool>("niryo_one/learning_mode", 10);
	publish_learning_mode_thread.reset(new std::thread(
			boost::bind(&RosInterface::publishLearningMode, this)));

	conveyor_1_feedback_publisher =
			nh_.advertise<niryo_one_msgs::ConveyorFeedback>(
					"niryo_one/kits/conveyor_1_feedback", 10);
	publish_conveyor_1_feedback_thread.reset(new std::thread(
			boost::bind(&RosInterface::publishConveyor1Feedback, this)));

	conveyor_2_feedback_publisher =
			nh_.advertise<niryo_one_msgs::ConveyorFeedback>(
					"niryo_one/kits/conveyor_2_feedback", 10);
	publish_conveyor_2_feedback_thread.reset(new std::thread(
			boost::bind(&RosInterface::publishConveyor2Feedback, this)));
}
