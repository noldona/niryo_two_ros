...
DxlDriver::DxlDriver(dynamixel::PortHandler *portHandler,
...
int DxlDriver::getModelNumber(uint8_t id, uint16_t *dxl_model_number) {
	...
	int result =
			packetHandler->ping(portHandler, id, dxl_model_number, &dxl_error);
...
int DxlDriver::syncWrite1Byte(uint8_t address, std::vector<uint8_t> &id_list,
		std::vector<uint32_t> &data_list) {
	dynamixel::GroupSyncWrite groupSyncWrite(
			portHandler, packetHandler, address, DXL_LEN_ONE_BYTE);
...
int DxlDriver::syncWrite2Bytes(uint8_t address, std::vector<uint8_t> &id_list,
		std::vector<uint32_t> &data_list) {
	dynamixel::GroupSyncWrite groupSyncWrite(
			portHandler, packetHandler, address, DXL_LEN_TWO_BYTES);
	...
	for (it_id = id_list.begin(), it_data = data_list.begin();
			it_id < id_list.end() && it_data < data_list.end();
			it_id++, it_data++) {
		uint8_t params[2] = {DXL_LOBYTE((uint16_t) (*it_data)),
				DXL_HIBYTE((uint16_t) (*it_data))};
		if (!groupSyncWrite.addParam(*it_id, params)) {
			groupSyncWrite.clearParam();
			return GROUP_SYNC_REDONDANT_ID;
		}
...
int DxlDriver::syncWrite4Bytes(uint8_t address, std::vector<uint8_t> &id_list,
		std::vector<uint32_t> &data_list) {
	dynamixel::GroupSyncWrite groupSyncWrite(
			portHandler, packetHandler, address, DXL_LEN_FOUR_BYTES);
...
int DxlDriver::read1Byte(uint8_t address, uint8_t id, uint32_t *data) {
	...
	dxl_comm_result = packetHandler->read1ByteTxRx(
			portHandler, id, address, &read_data, &dxl_error);
	(*data) = read_data;
...
int DxlDriver::syncRead(uint8_t address, uint8_t data_len,
		std::vector<uint8_t> &id_list, std::vector<uint32_t> &data_list) {
	data_list.clear();

	dynamixel::GroupSyncRead groupSyncRead(
			portHandler, packetHandler, address, data_len);
	...
	for (it_id = id_list.begin(); it_id < id_list.end(); it_id++) {
		dxl_getdata_result =
				groupSyncRead.isAvailable(*it_id, address, data_len);
		...
		if (data_len == DXL_LEN_ONE_BYTE) {
			data_list.push_back(
					(uint8_t) groupSyncRead.getData(*it_id, address, data_len));
		} else if (data_len == DXL_LEN_TWO_BYTES) {
			data_list.push_back((uint16_t) groupSyncRead.getData(
					*it_id, address, data_len));
		} else if (data_len == DXL_LEN_FOUR_BYTES) {
			data_list.push_back((uint32_t) groupSyncRead.getData(
					*it_id, address, data_len));
		}
	}
