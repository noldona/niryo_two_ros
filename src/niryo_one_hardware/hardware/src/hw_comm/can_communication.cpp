#include "niryo_one_hardware/can_communication.hpp"
...
int CanCommunication::init(int hardware_version, CanCommunicationConfig cfg) {
	...
	if (hardware_version != 1 && hardware_version != 2) {
		...
		RCLCPP_ERROR(rclcpp::get_logger("Can Communication"), "%s",
	...
	RCLCPP_INFO(rclcpp::get_logger("Can Communication"),
			"Writing data on CAN at %lf Hz", hw_write_frequency);
	RCLCPP_INFO(rclcpp::get_logger("Can Communication"),
			"Checking CAN connection at %lf Hz", hw_check_connection_frequency);
	...
	// start can driver
	can.reset(
			new NiryoCanDriver(spi_channel, spi_baudrate, gpio_can_interrupt));
	...
	required_steppers_ids = cfg.required_steppers_ids;
	allowed_steppers_ids = cfg.allowed_steppers_ids;
	required_steppers_ids.insert(required_steppers_ids.end(),
			required_steppers_ids.begin(), required_steppers_ids.end());
	allowed_steppers_ids.insert(allowed_steppers_ids.end(),
			required_steppers_ids.begin(), required_steppers_ids.end());
	allowed_steppers_ids.insert(this->allowed_steppers_ids.end(),
			allowed_steppers_ids.begin(), allowed_steppers_ids.end());

	RCLCPP_INFO(rclcpp::get_logger("Can Communication"),
			"Gear ratios : (1 : %lf, 2 : %lf, 3 : %lf, 4 : %lf, 6 : %lf, 7, "
			"%lf)",
			cfg.gear_ratio_1, cfg.gear_ratio_2, cfg.gear_ratio_3,
			cfg.gear_ratio_4, cfg.gear_ratio_6, cfg.gear_ratio_7);

	RCLCPP_INFO(rclcpp::get_logger("Can Communication"),
			"Home positions : (1 : %lf, 2 : %lf, 3 : %lf, 4 : %lf)",
			cfg.home_position_1, cfg.home_position_2, cfg.home_position_3,
			cfg.home_position_4);

	RCLCPP_INFO(rclcpp::get_logger("Can Communication"),
			"Angle offsets : (1 : %lf, 2 : %lf, 3 : %lf, 4 : %lf)",
			cfg.offset_position_1, cfg.offset_position_2, cfg.offset_position_3,
			cfg.offset_position_4);

	// Create motors with previous params
	m1 = StepperMotorState("Stepper Axis 1", CAN_MOTOR_1_ID, cfg.gear_ratio_1,
			cfg.direction_1,
			rad_pos_to_steps(cfg.home_position_1, cfg.gear_ratio_1,
					cfg.direction_1),  // home position
			rad_pos_to_steps(cfg.offset_position_1, cfg.gear_ratio_1,
					cfg.direction_1),  // offset position
			8, cfg.max_effort_1);
	m2 = StepperMotorState("Stepper Axis 2", CAN_MOTOR_2_ID, cfg.gear_ratio_2,
			cfg.direction_2,
			rad_pos_to_steps(
					cfg.home_position_2, cfg.gear_ratio_2, cfg.direction_2),
			rad_pos_to_steps(
					cfg.offset_position_2, cfg.gear_ratio_2, cfg.direction_2),
			8, cfg.max_effort_2);
	m3 = StepperMotorState("Stepper Axis 3", CAN_MOTOR_3_ID, cfg.gear_ratio_3,
			cfg.direction_3,
			rad_pos_to_steps(
					cfg.home_position_3, cfg.gear_ratio_3, cfg.direction_3),
			rad_pos_to_steps(
					cfg.offset_position_3, cfg.gear_ratio_3, cfg.direction_3),
			8, cfg.max_effort_3);
	...
	m4 = StepperMotorState("Stepper Axis 4", CAN_MOTOR_4_ID, cfg.gear_ratio_4,
			cfg.direction_4,
			rad_pos_to_steps(
					cfg.home_position_4, cfg.gear_ratio_4, cfg.direction_4),
			rad_pos_to_steps(
					cfg.offset_position_4, cfg.gear_ratio_4, cfg.direction_4),
			8, cfg.max_effort_4);
	// COnveyor belts steppers
	m6 = StepperMotorState("Stepper Conveyor belt 1 ", CAN_MOTOR_CONVEYOR_1_ID,
			cfg.gear_ratio_6, 1, 0, 0, 8, cfg.max_effort_6);
	m7 = StepperMotorState("Stepper Conveyor belt 2", CAN_MOTOR_CONVEYOR_2_ID,
			cfg.gear_ratio_7, 1, 0, 0, 8, cfg.max_effort_7);

	for (uint8_t i = 0; i < required_steppers_ids.size(); ++i) {
		if (required_steppers_ids.at(i) == m1.getId()) {
			m1.enable();
		} else if (required_steppers_ids.at(i) == m2.getId()) {
			m2.enable();
		} else if (required_steppers_ids.at(i) == m3.getId()) {
			m3.enable();
		}

		else if (hardware_version == 1 &&
				required_steppers_ids.at(i) == m4.getId()) {
			m4.enable();
		} else {
			debug_error_message = "Incorrect configuration : Wrong ID (" +
					std::to_string(required_steppers_ids.at(i)) +
					") given in Ros Param "
					"/niryo_one_motors/can_required_motors. You need to fix "
					"this !";
			RCLCPP_ERROR(rclcpp::get_logger("Can Communication"), "%s",
					debug_error_message.c_str());
			return -1;
	...
	if (required_steppers_ids.size() == 0) {
		debug_error_message = "Incorrect configuration : Ros Param "
							  "/niryo_one_motors/can_required_motors "
							  "should contain a list with at least one motor. "
	...
	RCLCPP_INFO(rclcpp::get_logger("Can Communication"),
			...
			(int) required_steppers_ids.size());
...
void CanCommunication::startHardwareControlLoop(bool limited_mode) {
	...
	if (!hardware_control_loop_thread) {
		RCLCPP_WARN(rclcpp::get_logger("Can Communication"),
				"START ctrl loop thread can");
		hardware_control_loop_thread.reset(new std::thread(
...
void CanCommunication::hardwareControlRead() {
	if (can->canReadData()) {
		...
		if (rxId >= 0x20) {
			...
			//RCLCPP_INFO(rclcpp::get_logger("Can Communication"), "frame  will be send somewhere else ");
			//return ;
		}
		...
		if ((motor_id == CAN_MOTOR_CONVEYOR_1_ID) &&
				(is_conveyor_id_1_connected)) {  //is_conveyor_id_1_connected = true; // if you want to ping // to test
			if ((update_id) & (old_id == motor_id)) {
				if (can->sendUpdateConveyorId(
							CAN_MOTOR_CONVEYOR_1_ID, new_id) != CAN_OK) {
					RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
							"Failed to send update conveyor with id : %d",
							motor_id);
				}
				update_id = false;
				is_conveyor_id_1_connected = false;
			...
			if (can->sendConveyoOnCommand(CAN_MOTOR_CONVEYOR_1_ID,
						is_conveyor_id_1_on, conveyor_id_1_speed,
						conveyor_id_1_direction) != CAN_OK) {
				RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
						"Failed to send command to the conveyor with id : %d",
						motor_id);
			}
		...
		if ((motor_id == CAN_MOTOR_CONVEYOR_2_ID) &&
				(is_conveyor_id_2_connected)) {  // is_conveyor_id_2_connected = true;  // if you want to ping conveyor
			if ((update_id) & (old_id == motor_id)) {
				if (can->sendUpdateConveyorId(
							CAN_MOTOR_CONVEYOR_2_ID, new_id) != CAN_OK) {
					RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
							"Failed to send update conveyor with id :%d",
							motor_id);
				}
			...
			if (can->sendConveyoOnCommand(CAN_MOTOR_CONVEYOR_2_ID,
						is_conveyor_id_2_on, conveyor_id_2_speed,
						conveyor_id_2_direction) != CAN_OK) {
				RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
						"Failed to send command to the conveyor with id : %d",
						motor_id);
			}
		...
		for (std::size_t i = 0; i < allowed_motors.size(); i++) {
			if (motor_id == allowed_motors.at(i)->getId()) {
				allowed_motors.at(i)->setLastTimeRead(clock.now().seconds());
				motor_found = true;
				break;
			}
		}
		...
		if (len < 1) {
			RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
					"Received can frame with empty data");
			return;
		}
		...
		if (control_byte == CAN_DATA_POSITION) {
			...
			int driver_temp = int(
					(-b - std::sqrt(b * b - 4 * a * (c - v_temp))) / (2 * a) +
					30);
			...
			//RCLCPP_INFO(rclcpp::get_logger("Can Communication"), "Mode : %d, Temp : %d", mode, m1.getTemperatureState());
			...
			RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
					"Received can frame with unknown control byte");
			return;
		}
	}
}
...
void CanCommunication::hardwareControlWrite() {
	if (clock.now().seconds() - time_hw_last_write > 1.0 / hw_write_frequency) {
		...
		if (write_position_enable) {
			for (std::size_t i = 0; i < motors.size(); i++) {
				if (motors.at(i)->isEnabled()) {
					if (can->sendPositionCommand(motors.at(i)->getId(),
								motors.at(i)->getPositionCommand()) != CAN_OK) {
						//RCLCPP_ERROR(rclcpp::get_logger("Can Communication"), "Failed to send position");
					}
				}
			}
		}
		...
		if (write_micro_steps_enable) {
			...
			for (std::size_t i = 0; i < motors.size(); i++) {
				if (motors.at(i)->isEnabled()) {
					if (can->sendMicroStepsCommand(motors.at(i)->getId(),
								motors.at(i)->getMicroStepsCommand()) !=
							CAN_OK) {
		...
		if (write_max_effort_enable) {
			...
			for (std::size_t i = 0; i < motors.size(); i++) {
				if (motors.at(i)->isEnabled()) {
					if (can->sendMaxEffortCommand(motors.at(i)->getId(),
								motors.at(i)->getMaxEffortCommand()) !=
							CAN_OK) {
...
void CanCommunication::hardwareControlCheckConnection() {
	if (clock.now().seconds() - time_hw_last_check_connection >
			1.0 / hw_check_connection_frequency) {
		...
		double time_now = clock.now().seconds();
		if (hw_check_connection_frequency >
				5.0) {  // if we check MCP_2515 too fast it will not work
			hw_check_connection_frequency = 5.0;
		}
		double timeout_read = 1.0 / hw_check_connection_frequency;
		int max_fail_counter = (int) (hw_check_connection_frequency +
				0.5);  // connection error will be detected after 1 sec

		for (std::size_t i = 0; i < motors.size(); i++) {
			if (motors.at(i)->isEnabled()) {
				if (time_now - motors.at(i)->getLastTimeRead() >
						timeout_read * (motors.at(i)->getHwFailCounter() + 1)) {
					RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
							"CAN connection problem with motor %d, hw fail "
							"counter : %d",
							motors.at(i)->getId(),
							motors.at(i)->getHwFailCounter());
					if (motors.at(i)->getHwFailCounter() >= max_fail_counter) {
						is_can_connection_ok = false;
						debug_error_message =
								"Connection problem with CAN bus. Motor ";
					...
					// reset MCP_2515
					//int result = can->init();
					//RCLCPP_INFO(rclcpp::get_logger("Can Communication"), "Can init : %d", result);
					motors.at(i)->setHwFailCounter(
							motors.at(i)->getHwFailCounter() + 1);
				} else {
...
void CanCommunication::hardwareControlLoop() {
	rclcpp::Rate hw_control_loop_rate = rclcpp::Rate(hw_control_loop_frequency);
	...
	while (rclcpp::ok()) {
		if (!hw_is_busy && hw_control_loop_keep_alive) {
			...
			resetHardwareControlLoopRates();
			// RCLCPP_INFO(rclcpp::get_logger("Can Communication"), "HW control loop, wait because is busy");
		}
	}
}
...
void CanCommunication::setTorqueOn(bool on) {
	if (!on && is_can_connection_ok && waiting_for_user_trigger_calibration &&
		...
		if (motors.at(i)->isEnabled()) {
			...
			if (firmware_version.length() == 0) {
				result_message = "No firmware version available for motor " +
						std::to_string(motors.at(i)->getId()) +
			...
			if (std::stoi(firmware_version.substr(0, 1)) < 2) {
				result_message =
						"You need to upgrade stepper firmware for motor " +
						std::to_string(motors.at(i)->getId());
				RCLCPP_WARN(rclcpp::get_logger("Can Communication"),
						"Can't process manual calibration : %s",
						result_message.c_str());
...
int CanCommunication::manualCalibration() {
	...
	for (std::size_t i = 0; i < motors.size(); i++) {
		if (motors.at(i)->isEnabled()) {
			...
			if (motors.at(i)->getId() == 1 || motors.at(i)->getId() == 2 ||
					motors.at(i)->getId() == 4) {  // position 0.0
			...
			RCLCPP_INFO(rclcpp::get_logger("Can Communication"),
					"Motor %d - sending offset : %d", motors.at(i)->getId(),
					offset_to_send);
			if (can->sendPositionOffsetCommand(motors.at(i)->getId(),
						offset_to_send,
...
int CanCommunication::sendCalibrationCommandForOneMotor(
	...
	if (can->sendCalibrationCommand(motor->getId(), motor->getOffsetPosition(),
				delay_between_steps,
...
int CanCommunication::getCalibrationResults(
		std::vector<StepperMotorState *> steppers, int calibr_timeout,
	...
	double time_begin_calibration = clock.now().seconds();
	double timeout = time_begin_calibration + (double) calibr_timeout;
	RCLCPP_INFO(rclcpp::get_logger("Can Communication"),
			"Waiting for motor calibration");
	//RCLCPP_INFO(rclcpp::get_logger("Can Communication"), "Waiting for motor %d calibration response...", motor->getId());

	while (clock.now().seconds() < timeout) {
		rclcpp::sleep_for(std::chrono::microseconds(500));  // check at 2000 Hz
			...
			for (std::size_t i = 0; i < motors_ids.size(); i++) {
				if (motors_ids.at(i) == motor_id) {
					if (len == 2) {
						...
						if (control_byte ==
								CAN_DATA_CALIBRATION_RESULT) {  // only check this frame
							int result = rxBuf[1];

							if (result == CAN_STEPPERS_CALIBRATION_TIMEOUT) {
								RCLCPP_ERROR(
										rclcpp::get_logger("Can Communication"),
							...
							4) {  // new firmware version -> get result + absolute sensor steps at offset position
						// 3. Check control byte
						int control_byte = rxBuf[0];
...
int CanCommunication::relativeMoveMotor(
	...
	if (can->sendTorqueOnCommand(motor->getId(), true) != CAN_OK) {
		RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
				"Failed to send torque ON to motor %d", motor->getId());
		return CAN_FAIL;
	}

	if (can->sendRelativeMoveCommand(motor->getId(), steps, delay) != CAN_OK) {
		RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
				"Relative Move motor failed for motor %d", motor->getId());
		return CAN_FAIL;
	}
	if (wait) {
		rclcpp::sleep_for(std::chrono::microseconds(
				(delay * steps) + 500000));  // wait for 0.5 sec more to finish
	}
...
int CanCommunication::autoCalibrationStep1() {
	...
	// 1. Move axis 3 up
	if (relativeMoveMotor(&m3,
				rad_pos_to_steps(0.5, m3.getGearRatio(), m3.getDirection()),
				1500, true) != CAN_OK) {
		return CAN_STEPPERS_CALIBRATION_FAIL;
	}
...
int CanCommunication::autoCalibrationStep2() {
	...
	if (sendCalibrationCommandForOneMotor(&m1, 800, 1, calibration_timeout) !=
			CAN_OK) {
		return CAN_STEPPERS_CALIBRATION_FAIL;
	}

	if (sendCalibrationCommandForOneMotor(&m2, 1100, 1, calibration_timeout) !=
			CAN_OK) {
		return CAN_STEPPERS_CALIBRATION_FAIL;
	}

	if (sendCalibrationCommandForOneMotor(&m4, 800, 1, calibration_timeout) !=
			CAN_OK) {
		return CAN_STEPPERS_CALIBRATION_FAIL;
	}
	...
	if (hardware_version == 2) {
		if (sendCalibrationCommandForOneMotor(
					&m3, 1100, -1, calibration_timeout) != CAN_OK) {
			return CAN_STEPPERS_CALIBRATION_FAIL;
		}
	}
	...
	if (getCalibrationResults(steppers, calibration_timeout, sensor_offset_ids,
				sensor_offset_steps) != CAN_STEPPERS_CALIBRATION_OK) {
		return CAN_STEPPERS_CALIBRATION_FAIL;
	}

	// 3. Move motor 1,2,4 to 0.0
	if (relativeMoveMotor(&m1, -m1.getOffsetPosition(), 1300, false) !=
			CAN_OK) {
		return CAN_STEPPERS_CALIBRATION_FAIL;
	}
	if (hardware_version == 1) {
		if (relativeMoveMotor(&m2, -m2.getOffsetPosition(), 3000, false) !=
				CAN_OK) {
			return CAN_STEPPERS_CALIBRATION_FAIL;
		}
	}
	if (relativeMoveMotor(&m4, -m4.getOffsetPosition(), 1500, false) !=
			CAN_OK) {
		return CAN_STEPPERS_CALIBRATION_FAIL;
	}
	...
	if (hardware_version == 1) {
		rclcpp::sleep_for(
				std::chrono::microseconds(abs(m4.getOffsetPosition()) * 1500));
	} else if (hardware_version == 2) {
		rclcpp::sleep_for(
				std::chrono::microseconds(abs(m1.getOffsetPosition()) * 1300));
	}
	...
	if (hardware_version == 2) {
		...
		rclcpp::sleep_for(std::chrono::microseconds(
				abs(m2.getOffsetPosition()) * 3000 + 500000));
	}
	...
	if (hardware_version == 1) {
		...
		if (getCalibrationResults(stepper, calibration_timeout,
					sensor_offset_ids,
					sensor_offset_steps) != CAN_STEPPERS_CALIBRATION_OK) {
			return CAN_STEPPERS_CALIBRATION_FAIL;
		}
...
void CanCommunication::setGoalPositionV1(double axis_1_pos_goal,
	...
	if (hardware_version == 1) {
		m1.setPositionCommand(rad_pos_to_steps(
				axis_1_pos_goal, m1.getGearRatio(), m1.getDirection()));
		m2.setPositionCommand(rad_pos_to_steps(
				axis_2_pos_goal, m2.getGearRatio(), m2.getDirection()));
		m3.setPositionCommand(rad_pos_to_steps(
				axis_3_pos_goal, m3.getGearRatio(), m3.getDirection()));
		m4.setPositionCommand(rad_pos_to_steps(
				axis_4_pos_goal, m4.getGearRatio(), m4.getDirection()));

		// if motor disabled, pos_state = pos_cmd (echo position)
		for (std::size_t i = 0; i < motors.size(); i++) {
			if (!motors.at(i)->isEnabled()) {
				motors.at(i)->setPositionState(
						motors.at(i)->getPositionCommand());
			}
		}
	}
}
...
void CanCommunication::getCurrentPositionV1(double *axis_1_pos,
	...
	if (hardware_version == 1) {
		*axis_1_pos = steps_to_rad_pos(
				m1.getPositionState(), m1.getGearRatio(), m1.getDirection());
		*axis_2_pos = steps_to_rad_pos(
				m2.getPositionState(), m2.getGearRatio(), m2.getDirection());
		*axis_3_pos = steps_to_rad_pos(
				m3.getPositionState(), m3.getGearRatio(), m3.getDirection());
		*axis_4_pos = steps_to_rad_pos(
				m4.getPositionState(), m4.getGearRatio(), m4.getDirection());
	}
}
...
void CanCommunication::setMicroSteps(std::vector<uint8_t> micro_steps_list) {
	...
	for (std::size_t i = 0; i < micro_steps_list.size(); i++) {
...
void CanCommunication::setMaxEffort(std::vector<uint8_t> max_effort_list) {
	...
	for (std::size_t i = 0; i < max_effort_list.size(); i++) {
...
void CanCommunication::getHardwareStatus(bool *is_connection_ok,
	...
	*(calibration_needed) =
			(waiting_for_user_trigger_calibration && is_can_connection_ok);
...
int CanCommunication::scanAndCheck() {
	...
	if (counter == 100) {
		debug_error_message =
				"Failed to scan motors, CAN bus is too busy. Will retry...";
		RCLCPP_WARN(rclcpp::get_logger("Can Communication"),
	...
	while (!m1_ok || !m2_ok || !m3_ok || !m4_ok || !m6_ok || !m7_ok ||
		...
		if (can->canReadData()) {
			...
			if (motor_id == m1.getId()) {
				...
				m3_ok = true;
					...
					motor_id == m4.getId()) {  // m4 only for Niryo One V1
				m4_ok = true;
			} else {  // detect unallowed motor
				RCLCPP_ERROR(rclcpp::get_logger("Can Communication"),
						"Scan CAN bus : Received can frame with wrong id : %d",
						motor_id);
				hw_is_busy = false;
				debug_error_message = "Unallowed connected motor : ";
				debug_error_message += std::to_string(motor_id);
		...
		if (clock.now().seconds() - time_begin_scan > timeout) {
			...
			if (!m1_ok) {
				debug_error_message += m1.getName();
				debug_error_message += ", ";
			}
			if (!m2_ok) {
				debug_error_message += m2.getName();
				debug_error_message += ", ";
			}
			if (!m3_ok) {
				debug_error_message += m3.getName();
				debug_error_message += ", ";
			}
			if (!m4_ok) {
				debug_error_message += m4.getName();
				debug_error_message += ", ";
			}
...
int CanCommunication::setConveyor(uint8_t id, bool activate) {
	if (id == CAN_MOTOR_CONVEYOR_1_ID) {
		...
		//RCLCPP_ERROR(rclcpp::get_logger("Can Communication"), " Received Conveyor wrong ID ");
...
int CanCommunication::updateConveyorId(uint8_t id, uint8_t new_id_up) {
	...
	if (((is_conveyor_id_1_connected) && (id == CAN_MOTOR_CONVEYOR_1_ID)) ||
			((is_conveyor_id_2_connected) && (id == CAN_MOTOR_CONVEYOR_2_ID))) {
		update_id = true;
...
void CanCommunication::getConveyorFeedBack(uint8_t conveyor_id,
	...
	//else RCLCPP_ERROR(rclcpp::get_logger("Can Communication"), " Received Conveyor wrong ID ");
}
