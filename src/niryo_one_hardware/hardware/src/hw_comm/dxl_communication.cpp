#include "niryo_one_hardware/dxl_communication.hpp"
...
int DxlCommunication::init(int hardware_version, DxlCommunicationConfig cfg) {
	...
	if (hardware_version != 1 && hardware_version != 2) {
		...
		RCLCPP_ERROR(rclcpp::get_logger("Dxl Communication"), "%s",
	...
	hw_data_write_frequency = cfg.hw_write_frequency;
	hw_data_read_frequency = cfg.hw_data_read_frequency;
	hw_status_read_frequency = cfg.hw_status_read_frequency;
	RCLCPP_INFO(rclcpp::get_logger("Dxl Communication"),
			"Start Dxl communication (%lf Hz)", hw_control_loop_frequency);
	RCLCPP_INFO(rclcpp::get_logger("Dxl Communication"),
			"Writing data on Dxl at %lf Hz", hw_data_write_frequency);
	RCLCPP_INFO(rclcpp::get_logger("Dxl Communication"),
			"Reading data from Dxl at %lf Hz", hw_data_read_frequency);
	RCLCPP_INFO(rclcpp::get_logger("Dxl Communication"),
	...
	required_motors_ids.insert(required_motors_ids.end(),
			cfg.required_dxl_ids.begin(), cfg.required_dxl_ids.end());
	allowed_motors_ids.insert(allowed_motors_ids.end(),
			cfg.required_dxl_ids.begin(), cfg.required_dxl_ids.end());
	allowed_motors_ids.insert(allowed_motors_ids.end(),
			cfg.allowed_dxl_ids.begin(), cfg.allowed_dxl_ids.end());

	// Create motors
	// hardware_version 1 : 2 motors for axis 5, 1 for axis 6, 1 for tool (all XL320)
	if (hardware_version == 1) {
		m5_1 = DxlMotorState("Servo Axis 5_1", DXL_MOTOR_5_1_ID,
				MOTOR_TYPE_XL320, XL320_MIDDLE_POSITION);
		m5_2 = DxlMotorState("Servo Axis 5_2", DXL_MOTOR_5_2_ID,
				MOTOR_TYPE_XL320, XL320_MIDDLE_POSITION);
	}
	// hardware_version 2 : 1 motor (XL430) for axis 4, 1 (XL430) for axis 5, 1 (XL320) for axis 6, 1 (XL320) for tool
		...
		m4 = DxlMotorState("Servo Axis 4", DXL_MOTOR_4_ID, MOTOR_TYPE_XL430,
				XL430_MIDDLE_POSITION);
		m5 = DxlMotorState("Servo Axis 5", DXL_MOTOR_5_ID, MOTOR_TYPE_XL430,
				XL430_MIDDLE_POSITION);
	}

	m6 = DxlMotorState("Servo Axis 6", DXL_MOTOR_6_ID, MOTOR_TYPE_XL320,
			XL320_MIDDLE_POSITION);

	// Enable motors
	if (hardware_version == 1) {
		for (std::size_t i = 0; i < cfg.required_dxl_ids.size(); i++) {
			if (cfg.required_dxl_ids.at(i) == m5_1.getId()) {
				m5_1.enable();
			} else if (cfg.required_dxl_ids.at(i) == m5_2.getId()) {
				m5_2.enable();
			} else if (cfg.required_dxl_ids.at(i) == m6.getId()) {
				m6.enable();
			} else {
				debug_error_message = "Incorrect configuration : Wrong ID (" +
						std::to_string(cfg.required_dxl_ids.at(i)) +
						") given in Ros Param "
						"/niryo_one_motors/dxl_required_motors. You need to "
						"fix this !";
				RCLCPP_ERROR(rclcpp::get_logger("Dxl Communication"), "%s",
						debug_error_message.c_str());
				return -1;
		...
		for (std::size_t i = 0; i < cfg.required_dxl_ids.size(); i++) {
			if (cfg.required_dxl_ids.at(i) == m4.getId()) {
				m4.enable();
			} else if (cfg.required_dxl_ids.at(i) == m5.getId()) {
				m5.enable();
			} else if (cfg.required_dxl_ids.at(i) == m6.getId()) {
				m6.enable();
			} else {
	...
	if (cfg.required_dxl_ids.size() == 0) {
	...
	tool = DxlMotorState(
			"No tool connected", 0, MOTOR_TYPE_XL320, XL320_MIDDLE_POSITION);
...
int DxlCommunication::setupCommunication() {
	RCLCPP_INFO(rclcpp::get_logger("Dxl Communication"),
			"Dxl : set port name (%s), baudrate(%d)", device_name.c_str(),
			uart_baudrate);
	...
	if (!dxlPortHandler->setupGpio()) {
		RCLCPP_ERROR(rclcpp::get_logger("Dxl Communication"),
				"Failed to setup direction GPIO pin for Dynamixel "
				"half-duplex serial");
...
void DxlCommunication::startHardwareControlLoop(bool limited_mode) {
	RCLCPP_INFO(rclcpp::get_logger("Dxl Communication"),
			"DXL : Start hardware control loop");
	...
	if (!hardware_control_loop_thread) {
		RCLCPP_WARN(rclcpp::get_logger("Dxl Communication"),
				"START ctrl loop thread dxl");
...
void DxlCommunication::hardwareControlRead() {
	...
	// read data
	if (clock.now().seconds() - time_hw_data_last_read >
		...
		if (read_position_enable) {
			...
			if (can_read_xl320) {
				...
				if (read_position_result == COMM_SUCCESS) {
					xl320_hw_fail_counter_read = 0;
					for (std::size_t i = 0; i < xl320_motor_list.size(); i++) {
			...
			if (can_read_xl430) {
				...
				if (read_position_result == COMM_SUCCESS) {
					xl430_hw_fail_counter_read = 0;
					for (std::size_t i = 0; i < xl430_motor_list.size(); i++) {
	...
	if (read_hw_status_enable) {
		if (clock.now().seconds() - time_hw_status_last_read >
...
void DxlCommunication::hardwareControlWrite() {
	...
	if (should_reboot_motors) {
		for (std::size_t i = 0; i < motors.size(); i++) {
			RCLCPP_WARN(rclcpp::get_logger("Dxl Communication"),
					"Reboot Dxl motor with ID: %d",
					(int) motors.at(i)->getId());
			xl430->reboot(motors.at(i)->getId());
		}
		if (tool.getId() != 0) {
			RCLCPP_WARN(rclcpp::get_logger("Dxl Communication"),
					"Reboot Dxl tool with ID: %d", (int) tool.getId());
			xl430->reboot(tool.getId());
		}
		should_reboot_motors = false;
	}

	if (clock.now().seconds() - time_hw_data_last_write >
		...
		if (torque_on) {
			...
			if (write_tool_enable && is_tool_connected) {
				...
				int write_tool_velocity_result = xl320->setGoalVelocity(
						tool.getId(), tool.getVelocityCommand());
				rclcpp::sleep_for(std::chrono::milliseconds(5));
				int write_tool_position_result = xl320->setGoalPosition(
						tool.getId(), tool.getPositionCommand());
				rclcpp::sleep_for(std::chrono::milliseconds(5));
				int write_tool_torque_result = xl320->setGoalTorque(
						tool.getId(), tool.getTorqueCommand());

				if (write_tool_velocity_result != COMM_SUCCESS ||
						write_tool_position_result != COMM_SUCCESS ||
						write_tool_torque_result != COMM_SUCCESS) {
					RCLCPP_WARN(rclcpp::get_logger("Dxl Communication"),
							"Failed to write on tool");
				} else {
					write_tool_enable =
		...
		if (write_led_enable) {
			...
			if (xl320_result != COMM_SUCCESS) {
				...
				write_led_enable =
						false;  // disable writing LED after success on all motors
			}
...
void DxlCommunication::hardwareControlLoop() {
	...
	while (rclcpp::ok()) {
		if (!hw_is_busy && hw_control_loop_keep_alive) {
			...
			resetHardwareControlLoopRates();
			// RCLCPP_INFO(rclcpp::get_logger("Dxl Communication"), "HW control loop, wait because is busy");
		}
	}
}
/*
...
void DxlCommunication::setGoalPositionV1(double axis_5_pos, double axis_6_pos) {
	if (hardware_version == 1) {
		...
		m5_2.setPositionCommand(
				XL320_MIDDLE_POSITION * 2 - m5_1.getPositionCommand());
...
void DxlCommunication::setGoalPositionV2(
	...
	if (hardware_version == 2) {
		...
		m5.setPositionCommand(
				XL430_MIDDLE_POSITION * 2 - rad_pos_to_xl430_pos(axis_5_pos));
		m6.setPositionCommand(rad_pos_to_xl320_pos(axis_6_pos));
...
void DxlCommunication::getCurrentPositionV1(
	...
	if (hardware_version == 1) {
		if (m5_1.isEnabled()) {
			...
			*axis_5_pos = xl320_pos_to_rad_pos(
					XL320_MIDDLE_POSITION * 2 - m5_2.getPositionState());
		}
...
void DxlCommunication::getCurrentPositionV2(
	...
	if (hardware_version == 2) {
		...
		*axis_5_pos = xl430_pos_to_rad_pos(
				XL430_MIDDLE_POSITION * 2 - m5.getPositionState());
		*axis_6_pos = xl320_pos_to_rad_pos(m6.getPositionState());
	}
}
...
void DxlCommunication::setLeds(std::vector<int> &leds) {
	if (leds.size() < motors.size() + 1) {
		RCLCPP_WARN(rclcpp::get_logger("Dxl Communication"),
				"Led array must contain %d values", (int) motors.size() + 1);
		return;
	}
...
int DxlCommunication::pingAndSetTool(uint8_t id, std::string name) {
	...
	RCLCPP_INFO(rclcpp::get_logger("Dxl Communication"),
			"Ping Tool : ping result for id (%d) : %d", id, ping_result);

	hw_is_busy = false;

	if (ping_result != COMM_SUCCESS) {
		RCLCPP_WARN(rclcpp::get_logger("Dxl Communication"),
				"Could not find tool with id: %d", id);
...
int DxlCommunication::openGripper(uint8_t id, uint16_t open_position,
	...
	int dxl_steps_to_do = abs(
			(int) open_position - (int) tool.getPositionState());  // position
	double seconds_to_wait =
			(double) dxl_steps_to_do / (double) dxl_speed;  // sec

	rclcpp::sleep_for(std::chrono::milliseconds(
			(int) (seconds_to_wait * 1000000) + 250000));
...
int DxlCommunication::closeGripper(uint8_t id, uint16_t close_position,
	...
	int dxl_steps_to_do = abs(
			(int) close_position - (int) tool.getPositionState());  // position
	double seconds_to_wait =
			(double) dxl_steps_to_do / (double) dxl_speed;  // sec
...
int DxlCommunication::pullAirVacuumPump(
	...
	int dxl_steps_to_do = abs((int) pull_air_position -
			(int) tool.getPositionState());  // position
	double seconds_to_wait =
			(double) dxl_steps_to_do / (double) dxl_speed;  // sec
...
int DxlCommunication::scanAndCheck() {
	...
	if (counter == 100) {
		debug_error_message = "Failed to scan motors, Dynamixel bus is too "
							  "busy. Will retry...";
		RCLCPP_WARN(rclcpp::get_logger("Dxl Communication"),
	...
	RCLCPP_INFO(rclcpp::get_logger("Dxl Communication"),
			"Dynamixel broadcast ping - Found IDs:");
	for (std::size_t i = 0; i < id_list.size(); i++) {
		RCLCPP_INFO(
				rclcpp::get_logger("Dxl Communication"), "- %d", id_list.at(i));
	}

	// 2. Check that ids correspond to niryo_one motors id list
	std::vector<uint8_t> missing_motor_ids;

	for (std::size_t i = 0; i < required_motors_ids.size(); i++) {
		if (std::find(id_list.begin(), id_list.end(),
					required_motors_ids.at(i)) == id_list.end()) {
	...
	if (missing_motor_ids.size() > 0) {
		...
		for (std::size_t i = 0; i < motors.size(); i++) {
			if (std::find(missing_motor_ids.begin(), missing_motor_ids.end(),
						motors.at(i)->getId()) != missing_motor_ids.end()) {
		...
		if (std::find(missing_motor_ids.begin(), missing_motor_ids.end(),
					tool.getId()) != missing_motor_ids.end()) {
