...
NiryoOneCommunication::NiryoOneCommunication(
		int hardware_version, NiryoOneCommunicationConfig cfg) {
	this->hardware_version = hardware_version;
	can_enabled = cfg.can_enabled;
	dxl_enabled = cfg.dxl_enabled;
	niryo_one_hw_check_connection_frequency =
			cfg.niryo_one_hw_check_connection_frequency;

	cfg_ = cfg;

	if (!can_enabled) {
		RCLCPP_WARN(rclcpp::get_logger("Niryo One Communication"),
...
int NiryoOneCommunication::init() {
	int result = 0;
	if (can_enabled) {
		result = canComm->init(hardware_version, cfg_.can_cfg);
		if (result != 0) {
			return result;
		}
	}
	if (dxl_enabled) {
		result = dxlComm->init(hardware_version, cfg_.dxl_cfg);
		if (result != 0) {
			return result;
		}
	}
...
void NiryoOneCommunication::manageCanConnectionLoop() {
	...
	while (rclcpp::ok()) {
		if (!canComm->isConnectionOk() || new_calibration_requested) {
			...
			RCLCPP_INFO(rclcpp::get_logger("Niryo One Communication"),
...
void NiryoOneCommunication::checkHardwareVersionFromDxlMotors() {
	...
	if (detected_version == 0) {
		...
		change_hardware_version_and_reboot(
				hardware_version, detected_version, cfg_.reboot);
	}
}
...
void NiryoOneCommunication::getHardwareStatus(bool *is_connection_ok,
	...
	if (can_enabled) {
		canComm->getHardwareStatus(&can_connection_ok, can_error_message,
	...
	if (dxl_enabled) {
		dxlComm->getHardwareStatus(&dxl_connection_ok, dxl_error_message,
...
void NiryoOneCommunication::getCurrentPosition(double pos[6]) {
	if (hardware_version == 1) {
		if (can_enabled) {
			canComm->getCurrentPositionV1(&pos[0], &pos[1], &pos[2], &pos[3]);
		}
		if (dxl_enabled) {
			dxlComm->getCurrentPositionV1(&pos[4], &pos[5]);
		}
		...
		if (can_enabled) {
			canComm->getCurrentPositionV2(&pos[0], &pos[1], &pos[2]);
		}
		if (dxl_enabled) {
			dxlComm->getCurrentPositionV2(&pos[3], &pos[4], &pos[5]);
		}
...
void NiryoOneCommunication::sendPositionToRobot(const double cmd[6]) {
	...
	if (!is_calibration_in_progress) {
		if (hardware_version == 1) {
			if (can_enabled) {
				canComm->setGoalPositionV1(cmd[0], cmd[1], cmd[2], cmd[3]);
			}
			if (dxl_enabled) {
				dxlComm->setGoalPositionV1(cmd[4], cmd[5]);
			}
			...
			if (can_enabled) {
				canComm->setGoalPositionV2(cmd[0], cmd[1], cmd[2]);
			}
			if (dxl_enabled) {
				dxlComm->setGoalPositionV2(cmd[3], cmd[4], cmd[5]);
			}
...
void NiryoOneCommunication::rebootMotors() {
	...
	if (dxl_enabled) {
		dxlComm->rebootMotors();
	}
}

void NiryoOneCommunication::activateLearningMode(bool activate) {
	if (can_enabled) {
		canComm->setTorqueOn(!activate);
	}
	if (dxl_enabled) {
		dxlComm->setTorqueOn(!activate);
	}
}
...
int NiryoOneCommunication::pullAirVacuumPump(
	...
	if (dxl_enabled) {
		return dxlComm->pullAirVacuumPump(
...
int NiryoOneCommunication::closeGripper(uint8_t id, uint16_t close_position,
	...
	if (dxl_enabled) {
		return dxlComm->closeGripper(id, close_position, close_speed,
