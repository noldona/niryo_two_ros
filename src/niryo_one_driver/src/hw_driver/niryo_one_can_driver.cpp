...
NiryoCanDriver::NiryoCanDriver(
	...
	this->mcp_can.reset(
			new MCP_CAN(spi_channel, spi_baudrate, gpio_can_interrupt));
}

bool NiryoCanDriver::setupInterruptGpio() {
	if (!this->mcp_can->setupInterruptGpio()) {
...
INT8U NiryoCanDriver::init() {
	...
	int result = this->mcp_can->begin(MCP_ANY, CAN_1000KBPS, MCP_16MHZ);
	RCLCPP_INFO(rclcpp::get_logger("NiryoCanDriver"), "Result begin can : %d",
			result);

	if (result != CAN_OK) {
		RCLCPP_ERROR(rclcpp::get_logger("NiryoCanDriver"),
...
bool NiryoCanDriver::canReadData() {
	return this->mcp_can->canReadData();
}

INT8U NiryoCanDriver::readMsgBuf(INT32U *id, INT8U *len, INT8U *buf) {
	return this->mcp_can->readMsgBuf(id, len, buf);
}
...
INT8U NiryoCanDriver::sendPositionCommand(int id, int cmd) {
	...
	return this->mcp_can->sendMsgBuf(id, 0, 4, data);
}
...
INT8U NiryoCanDriver::sendRelativeMoveCommand(int id, int steps, int delay) {
	uint8_t data[7] = {CAN_CMD_MOVE_REL, (uint8_t) ((steps >> 16) & 0xFF),
			...
			(uint8_t) (delay & 0XFF)};
	return this->mcp_can->sendMsgBuf(id, 0, 7, data);
}
...
INT8U NiryoCanDriver::sendTorqueOnCommand(int id, int torque_on) {
	...
	return this->mcp_can->sendMsgBuf(id, 0, 2, data);
}
...
INT8U NiryoCanDriver::sendUpdateConveyorId(uint8_t old_id, uint8_t new_id) {
	RCLCPP_ERROR(rclcpp::get_logger("NiryoCanDriver"), "new id : %d", new_id);
	uint8_t data[3] = {0};
	data[0] = CAN_CMD_MODE;
	data[1] = CAN_UPDATE_CONVEYOR_ID;
	data[2] = new_id;
	return this->mcp_can->sendMsgBuf(old_id, 0, 3, data);
}
...
INT8U NiryoCanDriver::sendPositionOffsetCommand(
	...
	return this->mcp_can->sendMsgBuf(id, 0, 6, data);
}
...
INT8U NiryoCanDriver::sendCalibrationCommand(
	...
	return this->mcp_can->sendMsgBuf(id, 0, 8, data);
}
