...
int XL430Driver::changeId(uint8_t id, uint8_t new_id) {
	return packetHandler->write1ByteTxOnly(
			portHandler, id, XL430_ADDR_ID, new_id);
}

int XL430Driver::changeBaudRate(uint8_t id, uint32_t new_baudrate) {
	return packetHandler->write1ByteTxOnly(
			portHandler, id, XL430_ADDR_BAUDRATE, (uint8_t) new_baudrate);
}

int XL430Driver::setLed(uint8_t id, uint32_t led_value) {
	return packetHandler->write1ByteTxOnly(
			portHandler, id, XL430_ADDR_LED, (uint8_t) led_value);
}

int XL430Driver::setTorqueEnable(uint8_t id, uint32_t torque_enable) {
	return packetHandler->write1ByteTxOnly(
			portHandler, id, XL430_ADDR_TORQUE_ENABLE, (uint8_t) torque_enable);
}

int XL430Driver::setGoalPosition(uint8_t id, uint32_t position) {
	return packetHandler->write4ByteTxOnly(
			portHandler, id, XL430_ADDR_GOAL_POSITION, position);
}

int XL430Driver::setGoalVelocity(uint8_t id, uint32_t velocity) {
	return packetHandler->write4ByteTxOnly(
			portHandler, id, XL430_ADDR_GOAL_VELOCITY, velocity);
}
...
int XL430Driver::setReturnDelayTime(uint8_t id, uint32_t return_delay_time) {
	return packetHandler->write1ByteTxOnly(portHandler, id,
			XL430_ADDR_RETURN_DELAY_TIME, (uint8_t) return_delay_time);
}

int XL430Driver::setLimitTemperature(uint8_t id, uint32_t temperature) {
	return packetHandler->write1ByteTxOnly(portHandler, id,
			XL430_ADDR_TEMPERATURE_LIMIT, (uint8_t) temperature);
}
...
int XL430Driver::setReturnLevel(uint8_t id, uint32_t return_level) {
	return packetHandler->write1ByteTxOnly(portHandler, id,
			XL430_ADDR_STATUS_RETURN_LEVEL, (uint8_t) return_level);
}

int XL430Driver::setAlarmShutdown(uint8_t id, uint32_t alarm_shutdown) {
	return packetHandler->write1ByteTxOnly(portHandler, id,
			XL430_ADDR_ALARM_SHUTDOWN, (uint8_t) alarm_shutdown);
}

int XL430Driver::customWrite(
	...
	if (byte_number == 1) {
		...
		return packetHandler->write4ByteTxOnly(
				portHandler, id, reg_address, value);
	} else {
		return -1;
	}
}
 ...
 *  -----------------   SYNC WRITE   --------------------
 */
int XL430Driver::syncWritePositionGoal(
...
int XL430Driver::syncWriteVelocityGoal(
...
int XL430Driver::syncWriteTorqueGoal(
		std::vector<uint8_t> &id_list, std::vector<uint32_t> &torque_list) {
	// No goal torque for this motor ?
	return COMM_TX_ERROR;
}

int XL430Driver::syncWriteTorqueEnable(std::vector<uint8_t> &id_list,
		std::vector<uint32_t> &torque_enable_list) {
	return syncWrite1Byte(
			XL430_ADDR_TORQUE_ENABLE, id_list, torque_enable_list);
}

int XL430Driver::syncWriteLed(
 ...
 *  -----------------   SYNC READ   --------------------
 */
int XL430Driver::syncReadPosition(
		std::vector<uint8_t> &id_list, std::vector<uint32_t> &position_list) {
	return syncRead(XL430_ADDR_PRESENT_POSITION, DXL_LEN_FOUR_BYTES, id_list,
			position_list);
}

int XL430Driver::syncReadVelocity(
		std::vector<uint8_t> &id_list, std::vector<uint32_t> &velocity_list) {
	return syncRead(XL430_ADDR_PRESENT_VELOCITY, DXL_LEN_FOUR_BYTES, id_list,
			velocity_list);
}

int XL430Driver::syncReadLoad(
		std::vector<uint8_t> &id_list, std::vector<uint32_t> &load_list) {
	return syncRead(
			XL430_ADDR_PRESENT_LOAD, DXL_LEN_TWO_BYTES, id_list, load_list);
}

int XL430Driver::syncReadTemperature(std::vector<uint8_t> &id_list,
		std::vector<uint32_t> &temperature_list) {
	return syncRead(XL430_ADDR_PRESENT_TEMPERATURE, DXL_LEN_ONE_BYTE, id_list,
			temperature_list);
}

int XL430Driver::syncReadVoltage(
		std::vector<uint8_t> &id_list, std::vector<uint32_t> &voltage_list) {
	return syncRead(XL430_ADDR_PRESENT_VOLTAGE, DXL_LEN_TWO_BYTES, id_list,
			voltage_list);
}

int XL430Driver::syncReadHwErrorStatus(
		std::vector<uint8_t> &id_list, std::vector<uint32_t> &hw_error_list) {
	return syncRead(XL430_ADDR_HW_ERROR_STATUS, DXL_LEN_ONE_BYTE, id_list,
			hw_error_list);
}
