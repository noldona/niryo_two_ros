...
class DxlMotorState {
	public:
	DxlMotorState() {}
	DxlMotorState(const std::string name, uint8_t id, int type,
	...
	void setId(uint8_t motor_id) {
		id = motor_id;
	}  // allows to change tool motor easily
	int getType() {
		return type;
	}
	void enable() {
	...
	uint32_t getVelocityState() {
		return state_vel;
	}
	uint32_t getTorqueState() {
		return state_torque;
	}
	...
	uint32_t getVoltageState() {
		return state_voltage;
	}
	uint32_t getHardwareErrorState() {
		return state_hw_error;
	}
	...
	void setVelocityState(uint32_t vel) {
		state_vel = vel;
	}
	void setTorqueState(uint32_t torque) {
		state_torque = torque;
	}
	...
	void setHardwareError(uint32_t hw_error) {
		state_hw_error = hw_error;
	}

	// getters - command
	uint32_t getPositionCommand() {
		return cmd_pos;
	}
	uint32_t getVelocityCommand() {
		return cmd_vel;
	}
	uint32_t getTorqueCommand() {
		return cmd_torque;
	}
	uint32_t getLedCommand() {
		return cmd_led;
	}

	// setters - command
	void setPositionCommand(uint32_t pos) {
		cmd_pos = pos;
	}
	void setVelocityCommand(uint32_t vel) {
		cmd_vel = vel;
	}
	void setTorqueCommand(uint32_t torque) {
		cmd_torque = torque;
	}
	void setLedCommand(uint32_t led) {
		cmd_led = led;
	}
