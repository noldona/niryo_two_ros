...
*********************************************************************************************************/
void MCP_CAN::mcp2515_readRegisterS(
		const INT8U address, INT8U values[], const INT8U n) {
	INT8U i;
...
*********************************************************************************************************/
void MCP_CAN::mcp2515_setRegisterS(
		const INT8U address, const INT8U values[], const INT8U n) {
	INT8U i;
...
*********************************************************************************************************/
void MCP_CAN::mcp2515_modifyRegister(
		const INT8U address, const INT8U mask, const INT8U data) {
...
INT8U MCP_CAN::mcp2515_setCANCTRL_Mode(const INT8U newmode) {
	...
	if (i == newmode) return MCP2515_OK;
...
void MCP_CAN::mcp2515_initCANBuffers(void) {
	...
	mcp2515_write_mf(
			MCP_RXM0SIDH, ext, ulMask); /*Set both masks to 0           */
	mcp2515_write_mf(
			MCP_RXM1SIDH, ext, ulMask); /*Mask register ignores ext bit */

	/* Set all filters to 0         */
	mcp2515_write_mf(
			MCP_RXF0SIDH, ext, ulFilt); /* RXB0: extended               */
	mcp2515_write_mf(
			MCP_RXF1SIDH, std, ulFilt); /* RXB1: standard               */
	mcp2515_write_mf(
			MCP_RXF2SIDH, ext, ulFilt); /* RXB2: extended               */
	mcp2515_write_mf(
			MCP_RXF3SIDH, std, ulFilt); /* RXB3: standard               */
...
INT8U MCP_CAN::mcp2515_init(
		const INT8U canIDMode, const INT8U canSpeed, const INT8U canClock) {
	INT8U res;
...
*********************************************************************************************************/
void MCP_CAN::mcp2515_write_id(
		const INT8U mcp_addr, const INT8U ext, const INT32U id) {
	uint16_t canid;
	INT8U tbufdata[4];
...
void MCP_CAN::mcp2515_write_canMsg(const INT8U buffer_sidh_addr) {
	...
	mcp2515_setRegisterS(
			mcp_addr + 5, m_nDta, m_nDlc); /* write data bytes             */

	if (m_nRtr == 1) /* if RTR set bit in byte       */
		m_nDlc |= MCP_RTR_MASK;

	mcp2515_setRegister(
			(mcp_addr + 4), m_nDlc); /* write the RTR and DLC        */
	mcp2515_write_id(
			mcp_addr, m_nExtFlg, m_nID); /* write CAN id                 */
}
...
MCP_CAN::MCP_CAN(int spi_channel, int spi_baudrate, INT8U gpio_can_interrupt) {
	...
	delay_spi_can.tv_nsec =
			5000L;  // wait 5 microseconds between 2 spi transfers
}
...
INT8U MCP_CAN::init_Mask(INT8U num, INT32U ulData) {
	...
	if ((num & 0x80000000) == 0x80000000) ext = 1;
...
*********************************************************************************************************/
INT8U MCP_CAN::setMsg(
		INT32U id, INT8U rtr, INT8U ext, INT8U len, INT8U *pData) {
	int i = 0;
	m_nID = id;
	m_nRtr = rtr;
	m_nExtFlg = ext;
	m_nDlc = len;
	for (i = 0; i < MAX_CHAR_IN_MESSAGE; i++) m_nDta[i] = *(pData + i);
...
INT8U MCP_CAN::clearMsg() {
	...
	m_nRtr = 0;
	m_nfilhit = 0;
	for (int i = 0; i < m_nDlc; i++) m_nDta[i] = 0x00;
...
INT8U MCP_CAN::sendMsg() {
	...
	do {
		...
		res1 = mcp2515_readRegister(
				txbuf_n - 1); /* read send buff ctrl reg 	*/
		res1 = res1 & 0x08;
...
INT8U MCP_CAN::sendMsgBuf(INT32U id, INT8U len, INT8U *buf) {
	...
	if ((id & 0x80000000) == 0x80000000) ext = 1;

	if ((id & 0x40000000) == 0x40000000) rtr = 1;
...
INT8U MCP_CAN::readMsgBuf(INT32U *id, INT8U *len, INT8U buf[]) {
	...
	if (m_nExtFlg) m_nID |= 0x80000000;

	if (m_nRtr) m_nID |= 0x40000000;
